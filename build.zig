...
fn linkHeadlessLibraries(b: *std.Build, exe: *std.Build.Step.Compile, useLocalDeps: bool) void {
...
pub fn build(b: *std.Build) !void {
    ...
    const headless = b.option(bool, "headless", "Build without GUI features") orelse false;
    ...
    const mainModule = if (!headless) b.addModule("main", .{
        .root_source_file = b.path("src/main.zig"),
        .target = target,
        .optimize = optimize,
    }) else b.addModule("main", .{
    ...
    if (!headless) {
        ...
        linkHeadlessLibraries(b, exe, useLocalDeps);
    }
